...
const CONNECTIONSTRING =
	...
	"mongodb+srv://Fabio:admin@cluster0.mvh5b.mongodb.net/5B?retryWrites=true&w=majority";
...
/*const whitelist = [
	...
	"http://localhost:1337", //Locale
	"https://fabio-cavallero-crud-server.herokuapp.com", //Eroku con https
	"http://fabio-cavallero-crud-server.herokuapp.com", //Eroku con http
];
//Verifica se l'origine dalla quale arriva la richiesta è valida (nella whitelist)
const corsOptions = {
...
app.use("/", function (req, res, next) {
	mongoClient.connect(CONNECTIONSTRING, function (err, client) {
		if (err) res.status(503).send("Errore di connessione al database");
		else {
...
//Lettura delle collezioni presenti nel db, messo sopra al listener GET, sennò non funziona, perchè questa è una listener specifica, a differenza di quelle sotto
app.get("/api/getCollections", function (req, res, next) {
	let db = req["client"].db(DBNAME) as _mongodb.Db;
	let request = db.listCollections().toArray(); //Elenco collezioni
...
app.get("/api/*", function (req, res) {
	...
	if (!id) {
		...
		let objectId = new _mongodb.ObjectId(id); //creo un nuovo oggetto nel formato corretto
...
app.post("/api/*", function (req, res) {
	let db = req["client"].db(DBNAME) as _mongodb.Db;
	let collection = db.collection(currentCollection);
	let request = collection.insertOne(req["body"]); //req[body] -> parametri contenuti nel body
	request.then(function (data) {
		res.send(data);
	});
	request.catch(function (err) {
